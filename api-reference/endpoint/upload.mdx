...
- **Metadata:** Metadata (camera, GPS location, timestamp) is stripped by default. For images, EXIF is removed from PNG, JPG, and WebP at any size, without re-encoding, so the pixels are untouched. Display orientation is preserved on JPG and WebP. For videos, container metadata is removed during the MP4 remux. Send `include-metadata: true` to keep it.
