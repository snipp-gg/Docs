...
| Ultra | No per-file limit |

PLUS and Ultra have no per-file cap. Individual uploads are only bounded by your weekly quota (20 GB on PLUS, 50 GB on Ultra). The absolute upload limit is **50 GB per file**. Files larger than your plan's limit return a `413` error.

<Info>
  Videos have a hard ceiling of **30 GB** per file on PLUS and Ultra; larger videos are rejected. Free accounts are still bound by the 50 MB per-file cap above.
</Info>
