...
- When every file in the request is rejected, the status is `400`. A multi-file request returns `error: "All files failed to upload."` alongside the same `failed` array; a single-file request returns the reason at the top level with `quota`, `used` and `resetsAt` beside it.
