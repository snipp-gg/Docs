...
- Files are capped at 50 MB on the free plan. Snipp+ and Ultra have no per-file cap, the same as [`/upload`](/relay-reference/endpoint/upload). Conversion runs inside the request, so a large video can take a while to come back.
